...
from builtin_metrics import (
    ...
    imbalance_ratio,
    stereotypical_ratio,
    stereotypical_ratio_combined,
    stereotypical_ratio_negative,
)
...
def generate_exact_confusion_matrices(
    ...
    *,
    ...
    The ``n=56`` case contains 553,270,671 rows and is not practical in a
    Streamlit session without precomputation.
    """
...
def load_confusion_matrices_from_pickle(path_or_bytes: str | Path | bytes) -> pd.DataFrame:
    """Load confusion matrices from a pickle file or uploaded bytes."""
...
def default_ratio_panel(total: int) -> list[float]:
    """Return five representative ratio panel values for a given total.
